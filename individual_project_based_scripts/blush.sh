...
awk -F ',' 'NR>59 {print $0}' "Animal_Experiments_Sequences_v1.csv" | while IFS=',' read -r col1 dataset_name project_name sub_project_name structural_name functional_name struc_coregistration roi_left roi_right histology physiology spio baseline_duration injection_duration _
do
...
# clear
    ...
    if [[ "$project_name" == "Project_BLusH_XC_SC" ]]; then
        ...
        Signal_Change_Map sm_despike_cleaned_mc_func.nii.gz "$datapath/$run_number" 15 1 $injection_duration_in_min
